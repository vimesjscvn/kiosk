...
namespace Admin.API.Domain.Services
{
    public class SettingService : ISettingService
    {
        ...
        {
            var device = await _unitOfWork.GetRepository<SettingDepartment>().GetAll().FirstOrDefaultAsync(x => x.Ip == ip);
            if (device == null)
            ...
            else
            {
                ...
                var valueType = await _unitOfWork.GetRepository<ListValueType>().FindAsync(x => x.Code == ValueTypeCode.DepartmentCode);
                var department = await _unitOfWork.GetRepository<ListValueExtend>().FindAsync(x => x.Code == device.Code && x.ListValueTypeId == valueType.Id);
        ...
        /// <returns></returns>
        public async Task<DeviceSettingResponse> CreateOrUpdateSettingDevice(CreateOrUpdateSettingDepartmentRequest request)
        {
            var setting = await _unitOfWork.GetRepository<SettingDepartment>().GetAll().FirstOrDefaultAsync(x => x.Ip == request.Ip);
            if (setting == null)
            {
        ...
        {
            Setting setting = await _unitOfWork.GetRepository<Setting>().FindAsync(x => x.Key == key);
        ...
        {
            Setting exsitedSetting = await _unitOfWork.GetRepository<Setting>().FindAsync(setting => setting.Key == request.Key);
            ...
            {
                var addSetting = _mapper.Map(new SettingViewModel { Key = request.Key, Value = ConvertSettingRequest(request) }, new Setting());
        ...
        public async Task<GetAllSettingResponse> GetAllSetting()
        {
            ...
            List<BaseSetting> listSetting = new List<BaseSetting>();
        ...
        {
            var generalSetting = await _unitOfWork.GetRepository<Setting>().FindAsync(x => x.Key == DefautSetting.DefautGeneralKey);
