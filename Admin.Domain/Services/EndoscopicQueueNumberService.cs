...
{
    public class EndoscopicQueueNumberService : IQueueNumberService
    {
        /// <summary>
        /// The unit of work
        /// </summary>
        ...
        public EndoscopicQueueNumberService(IUnitOfWork unitOfWork,
            ...
            ICollection<Reception> receptions)
        {
            ...
            string listNoNumberString = "B31000272,B31000273,B3100330,B3100331,B3100332,B3100333,B3100334,B3100335,B3100336,B3100337,B3100338,B3100339";
            var listNoNumber = listNoNumberString.Split(",").ToList();

            foreach (var reception in receptions)
            {
                var clinics = _unitOfWork.GetRepository<Clinic>().GetAll()
                               .Where(x => x.ReceptionId == reception.Id
                                       && !x.Reception.IsFinished && !listNoNumber.Contains(x.ServiceId)).ToList();
                if (clinics.Count > 0)
                {
            ...
            var serviceIds = services.Where(s => s.IsFinished == HISErrorCode.NotFinished).Select(s => s.ServiceExtendId);

            // 8. Lấy các phòng thực hiện dịch vụ
            var departmentServiceList = await _unitOfWork.GetRepository<CS.EF.Models.DepartmentService>().GetAll()
                                                                .Where(l => serviceIds.Any(m => m == l.ServiceId)).ToListAsync();
            ...
            // // var transactionClinics = transactionClinicList.DistinctBy(i => i.IdSpecified).ToList();
            ...
            var servicesNSDT = new List<ListValueExtend>();
            var servicesNSDTIds = new List<Guid>();
            var listValueTypeDV = await _unitOfWork.GetRepository<ListValueType>().GetAll().FirstOrDefaultAsync(x => x.Code == "DV");
            string listNSDTString = "B310000089,B310000092,B31000269,B3100267,B3100289,B3100290,B3100295,B3100296,B3100303,B3100304,B3100305,B3100312,B3100313,B3100314";
            var listNSDT = listNSDTString.Split(",").ToList();
            if (listValueTypeDV != null)
            {
                servicesNSDT = await _unitOfWork.GetRepository<ListValueExtend>().GetAll().Where(x => listNSDT.Contains(x.Code)
                    && x.ListValueTypeId == listValueTypeDV.Id).ToListAsync();
                servicesNSDTIds = servicesNSDT.Select(x => x.Id).ToList();
            }
            ...
            foreach (var reception in receptions)
            {
                ...
                var clinicItems = heldClinics.Where(clinic => clinic.ReceptionId == reception.Id)
                                                .GroupBy(i => new { i.DepartmentExtendId, i.ExaminationExtendId, i.ServiceExtendId, i.IdSpecified });
                var clinicNSDTList = heldClinics.Where(x => x.ReceptionId == reception.Id && servicesNSDTIds.Contains(x.ServiceExtendId.Value)).ToList();
                var clinicNSDTNotEnough3Hours = new List<ClinicNSDTNotEnough3Hours>();
                foreach (var clinicNSDT in clinicNSDTList)
                {
                    if (clinicNSDT.CreatedDate.AddHours(3) > DateTime.Now)
                    {
                        clinicNSDTNotEnough3Hours.Add(new ClinicNSDTNotEnough3Hours { ServiceId = clinicNSDT.ServiceExtendId, Enough3Hours = false, Time = clinicNSDT.CreatedDate.AddHours(3) });
                    }
                    else
                    {
                        clinicNSDTNotEnough3Hours.Add(new ClinicNSDTNotEnough3Hours { ServiceId = clinicNSDT.ServiceExtendId, Enough3Hours = true, Time = clinicNSDT.CreatedDate });
                    }
                }

                // var endoscopeDT = heldClinics.Where(clinic => clinic.ReceptionId == reception.Id && clinic.ser)

                string groupAnesthesiaString = "B3100289,B3100295,B3100312";
                List<string> groupsAnesthesia = groupAnesthesiaString.Split(",").ToList();

                bool hasAnesthesia = false;

                foreach (var transactionClinicItem in clinicItems)
                {
                    var service = _unitOfWork.GetRepository<ListValueExtend>().GetAll().FirstOrDefault(x => x.Id == transactionClinicItem.Key.ServiceExtendId);
                    if (service != null)
                    {
                        if (groupsAnesthesia.Contains(service.Code))
                        {
                            hasAnesthesia = true;
                        }
                    }
                ...
                foreach (var transactionClinicItem in clinicItems)
                {
                    ...
                    if (departmentService != null
                        && (departmentService.Room?.Code == "DIENTIM_202"
                        || departmentService.Room?.Code == "DIENTIM_201"))
                    {
                        if (patientInfo.Gender == Gender.Female)
                        {
                            departmentService = departmentServiceList.Where(x =>
                            transactionClinicItem.Key.ServiceExtendId == x.ServiceId
                            && x.Room?.Code == "DIENTIM_201").FirstOrDefault();
                        }
                        else
                        {
                            departmentService = departmentServiceList.Where(x =>
                            transactionClinicItem.Key.ServiceExtendId == x.ServiceId
                            && x.Room?.Code == "DIENTIM_202").FirstOrDefault();
                        }
                    }

                    if (departmentService != null
                        && (departmentService.Room?.Code == "NOISOI_251_260" || departmentService.Room?.Code == "NOISOI_254_255_260"))
                    {
                        if (hasAnesthesia)
                        {
                            departmentService = departmentServiceList.Where(x =>
                            transactionClinicItem.Key.ServiceExtendId == x.ServiceId
                            && x.Room?.Code == "NOISOI_254_255_260").FirstOrDefault();
                        }
                        else
                        {
                            departmentService = departmentServiceList.Where(x =>
                            transactionClinicItem.Key.ServiceExtendId == x.ServiceId
                            && x.Room?.Code == "NOISOI_251_260").FirstOrDefault();
                        }
                    }
                    ...
                    if (departmentService != null)
                    {
                        ...
                        numberData.DepartmentName = masterList.FirstOrDefault(m => m.Code == numberData.DepartmentCode)?.Description;
                        numberData.DepartmentId = departmentService.RoomId;

                        var queueNumberList = await _unitOfWork.GetRepository<QueueNumber>().GetAll().Where(l => roomCodes.Any(m => m == l.DepartmentExtendId)).ToListAsync();
                        ...
                        if (existNumber == null) // chưa có STT
                        {
                            ...
                            if (countQueueNumbers >= departmentConfig.TotalNumber)
                            {
                                ...
                                foreach (var item in transactionClinicItem)
                                {
                                    ...
                                    receptionJson.ExaminationId = item.ExaminationExtendId.HasValue ? item.ExaminationExtendId.Value : Guid.Empty;
                                ...
                                numberData.Time = now.ToString("yyyy-MM-dd HH:mm:ss");
                                departmentJson.Data.Add(numberData);

                                continue;
                            }

                            var clinicNSDT = clinicNSDTNotEnough3Hours.FirstOrDefault(x => x.ServiceId == transactionClinicItem.Key.ServiceExtendId);
                            if (clinicNSDT != null)
                            {
                                if (clinicNSDT.Enough3Hours == false)
                                {
                                    entity.DepartmentCode = departmentService.Room.Code;
                                    entity.PatientId = patientInfo.Id;
                                    //entity.PatientCode = patientInfo.Code;
                                    entity.Type = patientInfo.PatientType;
                                    // STT mặc định khi kham dai trang chua du 3 tieng dong ho
                                    entity.Number = -3;
                                    entity.AreaCode = _hospitalSettings.Hospital.HospitalArea;
                                    entity.DepartmentExtendId = departmentService.RoomId;
                                    ...
                                    numberData.Number = entity.Number;
                                    numberData.Time = clinicNSDT.Time.ToString("yyyy-MM-dd HH:mm:ss");
                                    departmentJson.Data.Add(numberData);
                        ...
                        else // Đã tồn tại STT
                        {
                            ...
                            // Truong hop kham dai trang da du 3 tieng dong ho
                            if (existNumber.Number == -3)
                            {
                                var existingClinicNSDT = clinicNSDTNotEnough3Hours.FirstOrDefault(x => x.ServiceId == transactionClinicItem.Key.ServiceExtendId);
                                if (existingClinicNSDT != null)
                                {
                                    if (existingClinicNSDT.Enough3Hours == true)
                                    {
                                        var predicate = PredicateBuilder.Create<QueueNumber>(q => q.DepartmentExtendId == existNumber.DepartmentExtendId);
                                        if (patientInfo.PatientType == PatientType.Normal)
                                        {
                                        ...
                                        var last = await _unitOfWork.GetRepository<QueueNumber>().GetAll().Where(x => x.Number != -3 && x.Number != -2 && x.Number != -1)
                                            .OrderByDescending(c => c.Number)
                                            .FirstOrDefaultAsync(predicate);
                                        ...
                                        numberData.Number = existNumber.Number;
                                        numberData.Time = existingClinicNSDT.Time.ToString("yyyy-MM-dd HH:mm:ss");
                                        departmentJson.Data.Add(numberData);
                                        continue;
                                    }
                                    else
                ...
                departmentJson.Data = departmentJson.Data.GroupBy(dep => new { dep.DepartmentId, dep.DepartmentCode, dep.DepartmentName, dep.Number, dep.Time })
