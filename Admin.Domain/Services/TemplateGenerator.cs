    ...
    public class TemplateGenerator : ITemplateGenerator
    {
          ...
          )
        {
            ...
            var dataGroupByEmloyee = data.GroupBy(i => new { i.EmployeeId, i.EmployeeName },
                (key, g) => new
                { EmployeeId = key.EmployeeId, EmployeeName = key.EmployeeName, Historys = g.ToList() }); ;
            var dataGroupByPaymentType = data.GroupBy(x => x.Manipulation).ToList();
            var base64Image = "";
            var numberGroup = dataGroupByEmloyee.Count();
            decimal? totalRecharged = data.Where(x => x.Manipulation == Constants.ManipulationContent.Recharged).Sum(x => x.Price);
            decimal? totalReturn = data.Where(x => x.Manipulation == Constants.ManipulationContent.Return).Sum(x => x.Price);
            decimal? totalCardFee = data.Where(x => x.Manipulation == Constants.ManipulationContent.CardFee).Sum(x => x.Price);
            decimal? totalForHospital = totalRecharged - totalReturn - totalCardFee;
            using (Image image = Image.FromFile(Path.Combine(webRoot, @"templates/tekmedi.png")))
            {
            ...
            sb.Append(@".tag-td {font-size: 12px;padding : 4px;text-align: center;background-color: red!important;color: white!important;}");
            sb.Append(@".tag-td-data {font-size: 8px;text-align: center;background-color: white!important;color: black!important;}");
            sb.Append(@".tag-td-data-date {padding : 10px;}");
            sb.Append(@".tag-td-title {font-size: 15px;text-align: left;background-color: white!important;color: black!important; font-weight: bold;}");
            sb.Append(@".sign {font-size: 12px;padding : 5px;text-align: center;background-color: white!important;color: black!important;");
            sb.Append(@"</style>");

            sb.Append(@"<header>");
            sb.Append(@"<div style='display:flex;'>");
            sb.AppendFormat(@"<img src=data:image/png;base64,{0} width =113 height = 83 hspace=51 vspace =13>", base64Image);
            sb.Append(@"<div width=113 height=83 hspace=51 vspace=13> <p> TEKMEDI JSC </p><p> The Sun Avenue SAV8 - 22.08, Số 28, Đường Mai Chí Thọ, Phường An Phú, Quận 2, Thành phố Hồ Chí Minh </p>");
            sb.Append(@"<p> www.tekmedi.com &nbsp&nbsp &nbsp   service @tekmedi.com &nbsp &nbsp &nbsp(028) 3 930 1439 </p></div></div></br></br>");
            ...
            for (var i = 0; i < numberGroup; i++)
            {
                ...
                sb.AppendFormat(@"<td class='tag-td-data' >{0}</td>", item.Historys.Count(h => h.Manipulation == Constants.ManipulationContent.Register));
                sb.AppendFormat(@"<td class='tag-td-data' >{0}</td>", item.Historys.Where(h => h.Manipulation == Constants.ManipulationContent.Register).Select(h => h.Price).Sum().ToString("#,##0"));
                sb.AppendFormat(@"<td class='tag-td-data' >{0}</td>", item.Historys.Count(h => h.Manipulation == Constants.ManipulationContent.Recharged));
                sb.AppendFormat(@"<td class='tag-td-data' >{0}</td>", item.Historys.Where(h => h.Manipulation == Constants.ManipulationContent.Recharged).Select(h => h.Price).Sum().ToString("#,##0"));
                sb.AppendFormat(@"<td class='tag-td-data' >{0}</td>", item.Historys.Count(h => h.Manipulation == Constants.ManipulationContent.Return));
                sb.AppendFormat(@"<td class='tag-td-data' >{0}</td>", item.Historys.Where(h => h.Manipulation == Constants.ManipulationContent.Return).Select(h => h.Price).Sum().ToString("#,##0"));
                sb.AppendFormat(@"<td class='tag-td-data' >{0}</td>", item.Historys.Count(h => h.Manipulation == Constants.ManipulationContent.CardFee));
                sb.AppendFormat(@"<td class='tag-td-data' >{0}</td>", item.Historys.Where(h => h.Manipulation == Constants.ManipulationContent.CardFee).Select(h => h.Price).Sum().ToString("#,##0"));
                sb.Append(@"</tr>");
            }
            sb.Append(@"</table>");
            ...
            for (var j = 0; j < dataGroupByPaymentType.Count(); j++)
            {
                ...
                sb.AppendFormat(@"<tr><td colspan='6' class='tag-td-title'>{0}</td>", item.Key == "Phí phát thẻ mới" ? "Mất thẻ".ToUpper() : item.Key.ToUpper());
            ...
            sb.Append(@"</br>");
            sb.AppendFormat(@"<span>Bệnh viện K, ngày {0} tháng {1} năm {2}</span>", DateTime.Now.Day, DateTime.Now.Month, DateTime.Now.Year);
            sb.Append(@"</div>");
            return sb.ToString();
        }
         ...
         )
        {
            ...
            sb.AppendFormat(@"<td class='tag-td-data' >{0}</td>", data.Count(h => h.Manipulation == Constants.ManipulationContent.Register));
            sb.AppendFormat(@"<td class='tag-td-data' >{0}</td>", data.Where(h => h.Manipulation == Constants.ManipulationContent.Register).Select(h => h.Price).Sum().ToString("#,##0"));
            sb.AppendFormat(@"<td class='tag-td-data' >{0}</td>", data.Count(h => h.Manipulation == Constants.ManipulationContent.Recharged));
            sb.AppendFormat(@"<td class='tag-td-data' >{0}</td>", data.Where(h => h.Manipulation == Constants.ManipulationContent.Recharged).Select(h => h.Price).Sum().ToString("#,##0"));
            sb.AppendFormat(@"<td class='tag-td-data' >{0}</td>", data.Count(h => h.Manipulation == Constants.ManipulationContent.Return));
            sb.AppendFormat(@"<td class='tag-td-data' >{0}</td>", data.Where(h => h.Manipulation == Constants.ManipulationContent.Return).Select(h => h.Price).Sum().ToString("#,##0"));
            sb.AppendFormat(@"<td class='tag-td-data' >{0}</td>", data.Count(h => h.Manipulation == Constants.ManipulationContent.CardFee));
            sb.AppendFormat(@"<td class='tag-td-data' >{0}</td>", data.Where(h => h.Manipulation == Constants.ManipulationContent.CardFee).Select(h => h.Price).Sum().ToString("#,##0"));
            sb.Append(@"</tr>");
            sb.Append(@"</table>");
         ...
         )
        {
            ...
            for (var j = 0; j < data.Count; j++)
            {
                ...
                sb.AppendFormat(@"<td class='tag-td-data' >{0}</td>", data[j].BirthdayYearOnly ? data[j].Birthday.Value.ToString("yyyy") : data[j].Birthday.Value.ToString("dd/MM/yyyy"));
                sb.Append(@"</tr>");
            }
            sb.Append(@"</table>");
            sb.Append(@"<div style='float: right;text-align: center;padding :10px;'>");
