...
namespace Core.Service.Implements
{
    public class EmailSenderService : IEmailSender
    {
        ...
        {
            mailRequest.PatientCode = StringUtils.MapValidPatientCode(mailRequest.PatientCode);
            ...
            var patient = await _unitOfWork.GetRepository<Patient>().GetAll()
                .FirstOrDefaultAsync(x => x.Code == mailRequest.PatientCode);
            if (patient == null)
            ...
            {
                var reception =
                    await _unitOfWork.GetRepository<Reception>().GetAsyncById(mailRequest.ReceptionId.Value);
                if (reception == null)
            ...
            if (mailRequest.EmailType == EmailType.QueueNumber)
            {
                var extendValue = await _unitOfWork.GetRepository<ListValueExtend>()
                    .FindAsync(x => x.Code == mailRequest.DepartmentCode);
                if (extendValue == null)
            ...
            throw new Exception(ErrorMessages.InvalidMailType);
        }
        ...
        public async Task<bool> VerifyCodeWithType(VerifyCodeRequest verifyRequest)
        {
            ...
            var patient = await _unitOfWork.GetRepository<Patient>()
                .FindAsync(x => x.Code == verifyRequest.PatientCode);
            if (patient == null)
                throw new Exception(ErrorMessages.NotFoundPatientCode);

            var isCompare = CompareCode(verifyRequest.VerifyCode, 10);

            if (isCompare)
            {
                if (verifyRequest.EmailType == EmailType.Reception) return true;

                if (verifyRequest.EmailType == EmailType.QueueNumber)
                {
                ...
                if (verifyRequest.EmailType == EmailType.SyncTransaction)
                {
                    var history = new History
                    {
                        Reason = verifyRequest.Reason,
                ...
                if (verifyRequest.EmailType == EmailType.CancelTransaction)
                {
                    var history = new History
                    {
                        Reason = verifyRequest.Reason,
        ...
        private async Task<bool> SendMailCompleteReception(SysUser sysUser, Reception reception, string reason,
            Patient patient)
        {
            var emailModel = new EmailSender();
            var emailSetting = await _unitOfWork.GetRepository<EmailSetting>().GetAll()
                .FirstOrDefaultAsync(x => x.EmailType == EmailType.Reception);
            ...
            var code = GenerateCode(patient.Code, emailSetting.TimeExpired);
            var tempPath = @"wwwroot/templates/CreateCompleteReceptionTemplate.html";
            var htmlBody = File.ReadAllText(tempPath);
        ...
        private async Task<bool> SendMailCreateQueueNumber(SysUser sysUser, ListValueExtend valueExtend, string reason,
            Patient patient)
        {
            var emailModel = new EmailSender();
            var emailSetting = await _unitOfWork.GetRepository<EmailSetting>().GetAll()
                .FirstOrDefaultAsync(x => x.EmailType == EmailType.QueueNumber);
        ...
        {
            int.TryParse(code.Substring(8), out var codeCompare);
            int.TryParse(DateTime.Now.AddMinutes(-timeExpired).ToString("MMddHHmmss"), out var now);
            if (now > codeCompare)
                return false;
            return true;
        }
        ...
        public static int GetDateTimeNow(int timeExpired)
        {
            ...
            int.TryParse(date.ToString("MMddHHmmss"), out var now);
            return now;
        }
    }
}