...
namespace Core.Service.Implements
{
    public class ExportExcelService : IExportExcelService
    {
        ...
        public ExportResponse ExportPatientRoom(List<PatientsInRoomViewModel> patientsInRoom)
        {
            ...
            var tempFile = new FileInfo(Path.Combine(webRoot, tempPath));
            ...
            {
                var detailSheet = package.Workbook.Worksheets["Chi Tiết"];
                ...
                if (resposne.Count > 0)
                {
                    ...
                    foreach (var item in resposne)
                    {
                        ...
                        detailSheet.Cells["F" + row].Value =
                            item.Birthday.HasValue ? item.Birthday.Value.ToString("dd/MM/yyyy") : "";
                        detailSheet.Cells["G" + row].Value = (int)item.Gender == 1 ? "Nam" : "Nữ";
                        detailSheet.Cells["H" + row].Value = (int)item.Type == 1 ? "Ưu tiên" : "Thường";
                        detailSheet.Cells["I" + row].Value = item.CreatedDate.HasValue
                            ? item.CreatedDate.Value.ToString("dd/MM/yyyy HH:mm:ss")
                            : "";
                        row++;
                    }
                    ...
                    detailSheet.Cells["K" + indexAddress].Value = "BV.Chợ Rẫy, Ngày " + DateTime.Now.ToString("dd") +
            ...
            using (var package = new ExcelPackage(file))
            {
                ...
                if (resposne.Count > 0)
                {
                    ...
                    foreach (var item in resposne)
                    {
                        ...
                        detailSheet.Cells["C" + row].Value = item.PatientCode;
                        detailSheet.Cells["D" + row].Value = item.FullName;
                        detailSheet.Cells["E" + row].Value = item.Birthday.HasValue
                        ...
                        detailSheet.Cells["H" + row].Value = item.CreatedDate.HasValue
                        ...
                        detailSheet.Cells["K" + row].Value = item.IsFinished ? "Đã hoàn thành" : "Chưa hoàn thành";
                        row++;
                    }
            ...
            using (var package = new ExcelPackage(file))
            {
                ...
                if (patients.Count() != 0)
                {
                    ...
                    foreach (var i in patients)
                    {
                        ...
                        detailSheet.Cells["H" + row].Value =
                            i.Birthday.HasValue ? i.Birthday.Value.ToString("dd/MM/yyyy") : string.Empty;
                    ...
                    var indexAddress = row + 1; // Next one rows
                    detailSheet.Cells["T" + indexAddress].Value = "BV.Chợ Rẫy, Ngày " + DateTime.Now.ToString("dd") +
                                                                  " tháng " + DateTime.Now.Month + " năm " +
                                                                  DateTime.Now.Year;
        ...
        public ExportResponse ExportBalance(ICollection<PatientInfoViewModel> patients, ExportRequest exportRequest)
        {
            ...
            using (var package = new ExcelPackage(file))
            {
                ...
                if (patients.Count() != 0)
                {
                    ...
                    var indexAddress = row + 1; // Next one rows
                    detailSheet.Cells["J" + indexAddress].Value = "BV.Chợ Rẫy, Ngày " + DateTime.Now.ToString("dd") +
                                                                  " tháng " + DateTime.Now.Month + " năm " +
                                                                  DateTime.Now.Year;
        ...
        public ExportResponse ExportDepartmentService(List<DepartmentServiceViewModel> departmentServices)
        {
            ...
            using (var package = new ExcelPackage(file))
            {
                ...
                if (departmentServices.Count() != 0)
                {
                    ...
                    var indexAddress = row + 1; // Next one rows
                    detailSheet.Cells["J" + indexAddress].Value = "Bệnh viện K, Ngày " + DateTime.Now.ToString("dd") +
                                                                  " tháng " + DateTime.Now.Month + " năm " +
                                                                  DateTime.Now.Year;
        ...
        public string GetFileImportError(List<ImportDepartmentServiceModel> datas)
        {
            ...
            using (var package = new ExcelPackage(file))
            {
                var detailSheet = package.Workbook.Worksheets[0];
                ...
                if (datas.Count > 0)
                {
                    ...
                    foreach (var item in datas)
                    {
                        if (!item.Status)
                        {
                            detailSheet.Cells[row, 1, row, 13].Style.Fill.PatternType = ExcelFillStyle.Solid;
                            detailSheet.Cells[row, 1, row, 13].Style.Fill.BackgroundColor
            ...
            using (var package = new ExcelPackage(file))
            {
                ...
                if (ocrSettings.Count() != 0)
                {
                    ...
                    foreach (var i in ocrSettings)
                    {
                        ...
                        detailSheet.Cells["E" + row].Value =
                            !string.IsNullOrEmpty(i.Value.AccessToken) ? i.Value.AccessToken : "";
                        detailSheet.Cells["F" + row].Value =
                            !string.IsNullOrEmpty(i.Value.ApiKey) ? i.Value.ApiKey : "";
                        detailSheet.Cells["G" + row].Value =
                            !string.IsNullOrEmpty(i.Value.TokenId) ? i.Value.TokenId : "";
                        detailSheet.Cells["H" + row].Value =
                            !string.IsNullOrEmpty(i.Value.TokenKey) ? i.Value.TokenKey : "";
