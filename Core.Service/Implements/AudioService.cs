...
namespace Core.Service.Implements
{
    public class AudioService : IAudioService
    {
        ...
        public string CreateAudio(string path, string table, int numberFrom, int numberTo, RoomType type,
            PriorityType typePatient)
        {
            var filePathExist = string.Format(@"{0}\{1}\{2}_{3}_{4}_number_result.wav", path, table, typePatient,
                numberFrom, numberTo);
            var fileName = string.Format(@"Audio\{0}\{1}_{2}_{3}_number_result.wav", table, typePatient, numberFrom,
                numberTo);
            ...
            try
            {
                ...
                if (arrFrom.Count == 0 && arrTo.Count == 0) return string.Empty;
        ...
        {
            var filePathExist = string.Format(@"{0}\{1}\{2}_{3}_{4}number_result.wav", path, table, type,
                virtualRoomCode, number);
            var pathName = string.Format(@"Audio\{0}\{1}_{2}_{3}number_result.wav", table, type, virtualRoomCode,
                number);
            ...
            try
            {
                ...
                var urlNumber = $"{path}\\number_{number}.wav";
                if (File.Exists(urlNumber)) listResult.Add(urlNumber);
            ...
            bool isRecall = false)
        {
            try
            {
                if (requestFrom <= 0 || requestTo <= 0) return "";

                var directoryPath = Path.Combine(webRoot, "Audio", "AudioTNB");
                if (!Directory.Exists(directoryPath)) Directory.CreateDirectory(directoryPath);

                var fileName =
                    $"table{requestTable}_code{departmentCode}_type{type}_from{requestFrom}_to{requestTo}.mp3";
                if (isRecall)
                    fileName =
                        $"table{requestTable}_code{departmentCode}_type{type}_from{requestFrom}_to{requestTo}_recall.mp3";

                var pathName = $"Audio/AudioTNB/{fileName}";
                var filePathExist = Path.Combine(directoryPath, fileName);

                if (File.Exists(filePathExist)) return pathName;

                var departmentText = string.Empty;
                if (departmentCode == "TATTOAN")
                    departmentText = "thanh toán";
                else if (departmentCode == "TNB") departmentText = "tiếp đón";

                var template =
                    $"Mời bệnh nhân {departmentText} từ số {requestFrom} đến số {requestTo} vào bàn số {requestTable}";
                ...
                {
                    template =
                        $"Mời bệnh nhân ưu tiên từ số {requestFrom} đến số {requestTo} vào bàn số {requestTable}";
                    if (requestFrom == requestTo)
                        template =
                            $"Mời bệnh nhân {departmentText} ưu tiên có số {requestFrom} vào bàn số {requestTable}";
                }
                ...
                using (var client = new WebClient())
                {
                    client.Headers.Add("Authorization",
                        "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6InZpbWVzIiwicm9sZSI6InN5c3RlbSIsImlhdCI6MTY5OTI1Njg4MH0.U_Cn6XIhgPFSieVloSZGMxofG-Nq8aja1MfcdhM5IFc");
            ...
            string patientName)
        {
            try
            {
                ...
                var directoryPath = Path.Combine(webRoot, "Audio", "AudioDept");

                if (!Directory.Exists(directoryPath)) Directory.CreateDirectory(directoryPath);

                var idx = DateTime.Now.ToString(DateTimeFormatConstants.DDMMYYYYNS);
                var fileName =
                    $"group{group}_code{departmentCode}_type{type}_from{requestFrom}_to{requestTo}_idx{idx}.mp3";
                var filePathExist = Path.Combine(directoryPath, fileName);

                var pathName = $"Audio/AudioDept/{fileName}";
                if (File.Exists(filePathExist)) return pathName;

                var template =
                    $"Mời bệnh nhân {patientName} từ số {requestFrom} đến số {requestTo} vào {departmentName}";
                if (requestFrom == requestTo)
