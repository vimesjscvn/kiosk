...
namespace Core.Service.Implements
{
    ...
    public class DiagnosingImageService : IDiagnosingImageService
    {
        ...
        public async Task<ICollection<QueueNumber>> CreateAndGenerateNumber(Patient patient,
            List<DiagnosingImageListModel> model)
        {
            var result = new List<QueueNumber>();
            var groupDeptCodes = model.Select(s => s.GroupDeptCode).ToList();

            var departments = await _unitOfWork.GetRepository<Department>().GetAll()
                .Where(d => groupDeptCodes.Any(code => code == d.GroupDeptCode)).ToListAsync();

            var updateListServiceRequest = new PostRawUpdateListServiceRequest();

            foreach (var item in model)
            {
                var departmentName = departments.FirstOrDefault(d => d.DepartmentCode == item.DepartmentCode)
                    ?.DepartmentName;
