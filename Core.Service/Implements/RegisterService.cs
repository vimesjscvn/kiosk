...
namespace Core.Service.Implements
{
    public class RegisterService : IRegisterService
    {
        ...
        public async Task UpdateResultListServiceOutPatient(UpdateResultListServiceRequest serviceRequest)
        {
            ...
            {
                var orders = await _unitOfWork.GetRepository<ServiceOrder>().GetAll()
                    .Where(so => so.OrderId == item.OrderId && !so.IsFinished).ToListAsync();
                ...
                var numbers = await _unitOfWork.GetRepository<QueueNumber>().GetAll()
                    .Where(so => so.OrderId == item.OrderId && !so.IsFinished).ToListAsync();
        ...
        public async Task<RegisterResponse> RegisterOutPatient(RegisterRequest request, Patient patient)
        {
            ...
            patient.PatientType =
                HealthcareHelper.GetPatientTypeWithCode(patient.HealthInsuranceNumber, patient.Birthday);
            var reception = await _unitOfWork.GetRepository<Reception>().FindAsync(x =>
                x.PatientCode == request.PatientCode && x.RegisteredCode == request.RegisteredCode);
            ...
            var department = await _unitOfWork.GetRepository<Department>().FindAsync(g =>
                g.DepartmentCode == departmentCode
            ...
            var deptGroup = await _unitOfWork.GetRepository<CS.EF.Models.DepartmentGroup>().FindAsync(g =>
                g.DepartmentCode == departmentCode
            ...
            else
            {
                ...
                serviceOrder.GroupServiceId = string.IsNullOrEmpty(request.GroupServiceId)
                                              ...
                                              && serviceOrder.ServiceId.Length > 5
                    ? serviceOrder.ServiceId.Substring(0, 5)
            ...
            if (number == null)
            {
                number = new QueueNumber
                {
                    ...
                    GroupCode = table != null && !string.IsNullOrEmpty(table.AreaCode) && table.AreaCode.Length > 0
                        ? table.AreaCode.Substring(table.AreaCode.Length - 1)
                        : string.Empty
                };
