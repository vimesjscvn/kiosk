...
namespace Core.Service.Implements
{
    ...
    public class DataService : IDataService
    {
        ...
        public async Task<ICollection<ServiceOrder>> GetListServiceByRegisteredCode(
            GetListServiceByRegisteredCodeModel model)
        {
            ...
            var response = await _hospitalSystemService.GetRawListServiceByRegisteredCode(
                new GetRawListServiceByRegisteredCodeRequest
                {
            ...
            if (response != null && response.Orders != null && response.Orders.Count > 0)
            {
                var reception = await _unitOfWork.GetRepository<Reception>()
                    .FindAsync(r => r.RegisteredCode == model.RegisteredCode);
                if (reception == null)
                {
                ...
                foreach (var item in response.Orders)
                {
                    ...
                    {
                        var dept =
                            _mapper.Map<GetRawListDepartmentServiceByRegisteredCodeData, ServiceDepartment>(deptItem);
                ...
                if (isAdded) await _unitOfWork.CommitAsync();
            }
        ...
        public async Task<ICollection<Group>> PostSyncListGroupDept(PostSyncListGroupDeptModel model)
        {
            ...
            foreach (var gItem in groupDepts)
            {
                ...
                var group = await _unitOfWork.GetRepository<Group>()
                    .FindAsync(g => g.GroupDeptCode == gItem.GroupDeptCode);
                if (group == null)
                {
                    group = gItem;
                ...
                {
                    var dept = await _unitOfWork.GetRepository<Department>().FindAsync(d =>
                        d.GroupDeptCode == dItem.GroupDeptCode
                        && d.DepartmentCode == dItem.DepartmentCode);

                    var deptGroup = await _unitOfWork.GetRepository<CS.EF.Models.DepartmentGroup>().FindAsync(d =>
                        d.GroupCode == dItem.GroupDeptCode
                        && d.DepartmentCode == dItem.DepartmentCode);

                    if (dept != null)
                    {
                        depts.Add(dept);
                        if (deptGroup != null) continue;

                        deptGroup = new CS.EF.Models.DepartmentGroup
                        {
                            Id = Guid.NewGuid(),
                        ...
                        _unitOfWork.GetRepository<CS.EF.Models.DepartmentGroup>().Add(deptGroup);
                    }
            ...
            {
                var deptConfig = await _unitOfWork.GetRepository<DepartmentConfig>().FindAsync(d =>
                    d.GroupDeptCode == dept.GroupDeptCode
                    && d.DepartmentCode == dept.DepartmentCode);

                if (deptConfig != null) continue;
                ...
                deptConfig = new DepartmentConfig
                {
                    ...
                    AgeTo = 100,
                    TimeActive =
                        "{\"EndTimeActiveAM\":\"12:00:00\",\"StartTimeActiveAM\":\"05:00:00\",\"EndTimeActivePM\":\"16:00:00\",\"StartTimeActivePM\":\"13:00:00\"}",
                    TimeOnMinute = 9,
                    TotalNumber = 65,
                    TypeGender = TypeGender.All,
                    ActiveReExamDays = new[] { 0, 1, 2, 3, 4, 5, 6 }
                };
        ...
        private async Task<Group> InsertMasterData(Group gItem)
        {
            ...
            var listValue = await _unitOfWork.GetRepository<ListValue>().FindAsync(g =>
                g.ListValueTypeId == listValueType.Id
            ...
            if (listValueType != null)
            {
                ...
                {
                    var listValueExtend = await _unitOfWork.GetRepository<ListValueExtend>().FindAsync(d =>
                        d.ListValueId == listValue.Id
                        && d.Code == dItem.DepartmentCode);

                    if (listValueExtend != null) continue;
