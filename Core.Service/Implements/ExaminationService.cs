...
namespace Core.Service.Implements
{
    ...
    public class ExaminationService : IExaminationService
    {
        ...
        public async Task<ICollection<Reception>> GetListByIdentityCardOrHealthcareNumber(string code)
        {
            ...
            var response = await _hospitalSystemService.GetRawListExaminationByCode(
                new GetRawListExaminationByCodeRequest
                {
            ...
            {
                // Find patient in Tekmedi System
                var healthInsuranceNumberResponse = await _hospitalSystemService.GetRawListExaminationByCode(
                    new GetRawListExaminationByCodeRequest
                    {
            ...
            {
                var patientInfo = await _unitOfWork.GetRepository<Patient>().GetAll()
                    .FirstOrDefaultAsync(p => p.Code == item.PatientCode);

                if (patientInfo == null)
                {
                    // Find patient in Tekmedi System
                    var patientRes = await _hospitalSystemService.GetRawPatientByRegisteredCode(
                ...
                var reception = await _unitOfWork.GetRepository<Reception>().FindAsync(r =>
                    r.PatientCode == item.PatientCode
            ...
            return await _unitOfWork.GetRepository<Reception>().GetAll().Where(r => r.PatientCode == patientCode)
                .ToListAsync();
        }
        ...
        public async Task<ICollection<Reception>> GetListByTypeAndCode(string type, string code)
        {
            ...
            switch (type)
            {
                ...
                case DocumentTypes.BHYT:
                    var pHealthModel = await _unitOfWork.GetRepository<Patient>().GetAll().AsNoTracking()
                        .Where(p => p.HealthInsuranceNumber.Contains(code)).FirstOrDefaultAsync();
                ...
                case DocumentTypes.CCCD:
                    var pIdentityModel = await _unitOfWork.GetRepository<Patient>().GetAll().AsNoTracking()
                        .Where(p => p.IdentityCardNumber.Contains(code)).FirstOrDefaultAsync();
        ...
        {
            var patientInfo =
                await _unitOfWork.GetRepository<Patient>().GetAll().FirstOrDefaultAsync(p => p.Code == code);
            ...
            var registeredCode = response != null
                ...
                ? response.Result.FirstOrDefault()?.RegisteredCode
                : string.Empty;
            ...
            if (!string.IsNullOrEmpty(registeredCode) && patientInfo == null)
            {
                ...
                patientInfo = await _unitOfWork.GetRepository<Patient>().GetAll()
                    .FirstOrDefaultAsync(p => p.Code == patientRes.PatientCode);
            ...
            if (response.Result.Count == 0 || patientInfo == null) return new List<Reception>();
        ...
        public async Task<PatientExaminationDetail> GetDetailByRegisteredCode(string code)
        {
            ...
            if (reception == null) throw new Exception(ErrorMessages.NotFoundExamination);
                ...
                if (examinationRes.Result.TestResults != null && examinationRes.Result.TestResults.Count > 0)
                {
                    ...
                    foreach (var groupItem in groups)
                    {
                        var serviceGroup = new PatientClinicGroupDetail
                        {
                            ...
                            ServiceTypeName = examinationRes.Result.TestResults
                                .FirstOrDefault(t => t.ServiceTypeId == groupItem.Key.ServiceTypeId)?.ServiceTypeName,
                            RegisteredDate = reception.RegisteredDate,
                            RegisteredCode = reception.RegisteredCode,
        ...
        public async Task<PatientExaminationServiceGroupDetail> GetServiceDetailByRegisteredCodeAndServiceId(
            string code, string typeId)
        {
            var reception = await _unitOfWork.GetRepository<Reception>().FindAsync(r => r.RegisteredCode == code);
            ...
            if (examinationRes.Result != null)
            {
                ...
                {
                    var groups = examinationRes.Result.TestResults.GroupBy(g => new { g.ServiceTypeId, g.OrderId })
                        .Where(w => w.Key.ServiceTypeId + "-" + w.Key.OrderId == typeId).FirstOrDefault();
                    if (groups != null)
                    {
                        ...
                        var isTestGroup = !string.IsNullOrEmpty(detail.ServiceTypeName) &&
                                          detail.ServiceTypeName.ToLower().IndexOf(ServiceGroups.XN) > 0;
                        if (!isTestGroup)
                        {
        ...
        public async Task<PatientExaminationServiceGroupDetail> GetServiceDetailByOrder(string code, string orderId,
            string orderLineId)
        {
            var reception = await _unitOfWork.GetRepository<Reception>().FindAsync(r => r.RegisteredCode == code);
        ...
        public async Task<ICollection<QueueNumber>> Create(Patient patient, List<ExamRegistrationListModel> model,
            bool isThrowError = false, string registrationType = "")
        {
            ...
            var resp = await _hospitalSystemService.PostRawRegisterExamination(new PostRawRegisterExaminationRequest
            {
                ...
                Birthday = patient.Birthday.ToString(DateTimeFormatConstants.YYYYMMDDTHHMMSSZ,
                ...
                HealthInsuranceExpiredDate =
                    patient.HealthInsuranceExpiredDate?.ToString(DateTimeFormatConstants.YYYYMMDDTHHMMSSZ,
                        CultureInfo.InvariantCulture),
                ...
                HealthInsuranceIssuedDate =
                    patient.HealthInsuranceIssuedDate?.ToString(DateTimeFormatConstants.YYYYMMDDTHHMMSSZ,
                        CultureInfo.InvariantCulture),
                HealthInsuranceNumber = patient.HealthInsuranceNumber,
                IdentityCardNumber = patient.IdentityCardNumber,
                IdentityCardNumberIssuedDate =
                    patient.IdentityCardNumberIssuedDate?.ToString(DateTimeFormatConstants.YYYYMMDDTHHMMSSZ,
                        CultureInfo.InvariantCulture),
                ...
                Departments = model
                    .Select(_mapper.Map<ExamRegistrationListModel, PostRawRegisterExaminationDataRequest>).ToList()
            });
            ...
            if (resp != null)
            {
                if (resp.Result != null && resp.Result.Count > 0)
                {
                    ...
                    {
                        var number = await _unitOfWork.GetRepository<QueueNumber>().FindAsync(r =>
                        ...
                        var reception = await _unitOfWork.GetRepository<Reception>()
                            .FindAsync(r => r.RegisteredCode == item.RegisteredCode);
                        if (reception == null)
                        {
                            reception = new Reception
                            {
        ...
        public async Task<ICollection<QueueNumber>> CreateAndGenerateNumber(Patient patient,
            List<ExamRegistrationListModel> model)
        {
            var result = new List<QueueNumber>();
        ...
        public async Task<ICollection<QueueNumber>> CreateWithGroup(Patient patient,
            List<ExamRegistrationListModel> model, bool isThrowError = false)
        {
            var ip = model.FirstOrDefault()?.Ip;
            var exams = new List<PostRawRegisterExamByGroupResponseData>();
            var receptionCodes = await _unitOfWork.GetRepository<Reception>().GetAll().Where(r =>
                    r.PatientCode == patient.Code
                    && DateTime.Compare(r.RegisteredDate.Date, DateTime.Now.Date) == 0).Select(s => s.RegisteredCode)
                .Distinct().ToListAsync();

            var responses = new List<PostRawRegisterExamByGroupResponse>();
            foreach (var item in model)
            {
                var groupExams = await _hospitalSystemService.PostRawRegisterExamByGroup(
                    new PostRawRegisterExamByGroupRequest
                    {
                        PatientCode = patient.Code,
            ...
            foreach (var item in exams)
            {
                ...
                var reception = await _unitOfWork.GetRepository<Reception>().GetAll()
                    .FirstOrDefaultAsync(r => r.RegisteredCode == item.RegisteredCode);
                if (reception == null)
                {
                    reception = new Reception
                    {
        ...
        private async Task<string> ValidateHealthInsurance(Patient patient)
        {
            ...
            else if (!string.IsNullOrEmpty(patient.IdentityCardNumber)) insuranceCode = patient.IdentityCardNumber;
