...
namespace Core.Service.Implements
{
    ...
    public class PatientService : IPatientService
    {
        ...
        public Patient Update(Patient entity, Guid id)
        {
            ...
            var existing = _unitOfWork.GetRepository<Patient>().GetById(id);
            if (existing != null)
            {
        ...
        public async Task<Patient> UpdateAsync(Patient entity, Guid id)
        {
            ...
            var existing = await _unitOfWork.GetRepository<Patient>().GetAsyncById(id);
            if (existing != null)
            {
                _unitOfWork.GetRepository<Patient>().Update(entity);
                await _unitOfWork.CommitAsync();
        ...
        public async Task<Patient> GetByPatientCode(string code)
        {
            ...
            if (patient == null)
            {
                ...
                patient = await _unitOfWork.GetRepository<Patient>().GetAll()
                    .FirstOrDefaultAsync(item => item.Code == response.PatientCode);
        ...
        {
            var patient = await _unitOfWork.GetRepository<Patient>().GetAll()
                .FirstOrDefaultAsync(item => item.HealthInsuranceNumber == healthInsuranceNumber);
            return patient;
        }
        ...
        public async Task<Patient> GetByRegisteredCode(string registeredCode)
        {
            var reception = await _unitOfWork.GetRepository<Reception>().GetAll()
                .FirstOrDefaultAsync(item => item.RegisteredCode == registeredCode);
            if (reception == null)
            {
                // Find patient in Tekmedi System
                var response = await _hospitalSystemService.GetRawPatientByRegisteredCode(
                    new GetRawPatientByRegisteredCodeRequest
                    {
                        RegisteredCode = registeredCode
                    });

                var patientRes = await _unitOfWork.GetRepository<Patient>().GetAll()
                    .FirstOrDefaultAsync(item => item.Code == response.PatientCode);

                if (patientRes != null && !string.IsNullOrEmpty(patientRes.Code)) return patientRes;
            ...
            var patient = await _unitOfWork.GetRepository<Patient>().GetAll()
                .FirstOrDefaultAsync(item => item.Code == reception.PatientCode);
            return patient;
        }
        ...
        {
            var patient = await _unitOfWork.GetRepository<Patient>().GetAll()
                .FirstOrDefaultAsync(item => item.Code == request.PatientCode);
        ...
        public async Task<ICollection<ExaminationCalendar>> GetCalendarByDate(GetCalendarByDateRequest request)
        {
            ...
            foreach (var day in DateUtils.EachCalendarDay(request.StartDate, request.EndDate))
            {
                ...
                if (response != null && response.Count > 0) responses.AddRange(response);
            }
        ...
        public async Task<TempPatient> GetPatientByQRCode(string code, List<Functions> functionsIds)
        {
            ...
            bool verifyQrCode(string qrCode)
            {
                ...
                if (qrCodeParts.Count() < 15) return false;

                if (qrCodeParts[0].Length != 10 && qrCodeParts[0].Length != 15) return false;

                if (qrCodeParts[1].Length == 0) return false;
                ...
                try
                {
                    ...
                    {
                        var date = DateTime.ParseExact(qrCodeParts[2], "dd/MM/yyyy", CultureInfo.InvariantCulture);
                    }
                }
                catch (Exception ex)
                {
            ...
            if (!verifyQrCode(code)) throw new Exception(ErrorMessages.InvalidQRCode);
            ...
            // Find patient in Tekmedi System
            var tokenResponse = await _insuranceGatewayService.PostRawLoginInsuranceGateway(
                new PostRawLoginInsuranceGatewayRequest
                {
                    username = _insuranceSettings.Insurance.Username,
                    password = EncryptingHelper.MD5Hash(_insuranceSettings.Insurance.Password)
                });

            var verifyResponse = await _insuranceGatewayService.PostRawVerifyInsuranceGateway(
                new PostRawVerifyInsuranceGatewayRequest
                {
            ...
            if (verifyResponse.maKetQua == "001") return await GetPatientByQRCodeWithNoCheckSocialGateway(code);
            ...
            if (isDash && addressParts.Length < 3) addressParts = address.Split("-");
            ...
            for (var i = addressParts.Length - 1; i >= 0; i--)
            {
                ...
                address = !string.IsNullOrEmpty(location.ProvinceCode) ? string.Join("", addressParts.Where(s => s != "")) : "";
            }

            if (!string.IsNullOrEmpty(insuranceCode))
            {
                patient = await _unitOfWork.GetRepository<Patient>().GetAll()
                    .FirstOrDefaultAsync(item => item.HealthInsuranceNumber == insuranceCode);
                ...
                if (patient != null)
                {
                    ...
                    patient.HealthInsuranceIssuedDate = healthInsuranceIssuedDate.Length == 10
                        ? DateTime.ParseExact(healthInsuranceIssuedDate, DateTimeFormatConstants.DDMMYYYY,
                            CultureInfo.InvariantCulture)
                        : DateTime.Now;
                    patient.HealthInsuranceExpiredDate = DateTime.ParseExact(healthInsuranceExpiredDate,
            ...
            if (isSyncPatient)
            {
                ...
                if (patient == null)
                {
                    ...
                    if (patient == null)
                        if (!string.IsNullOrEmpty(verifyResponse.maTheMoi) &&
                            verifyResponse.maTheMoi != verifyResponse.maThe)
            ...
            return new TempPatient
            {
                ...
                HealthInsuranceExpiredDate = !string.IsNullOrEmpty(healthInsuranceExpiredDate)
                    ...
                    ? DateTime.ParseExact(healthInsuranceExpiredDate, DateTimeFormatConstants.DDMMYYYY,
                        CultureInfo.InvariantCulture)
                    : DateTime.Now,
                LastName = name.Split(" ")[0],
                FirstName = string.Join(" ", name.Split(" ").Where((s, i) => i > 0)),
        ...
        public async Task<TempPatient> GetPatientByNationalIdQRCode(string code, List<Functions> functionsIds)
        {
            ...
            bool verifyQrCode(string qrCode)
            {
                ...
                var qrCodeParts = qrCode.Split("|");
                if (qrCodeParts.Count() < 7) return false;

                if (qrCodeParts[0].Length != 12) return false;

                try
                {
                    if (qrCodeParts[3].Length == 8)
                    {
                        var date = DateTime.ParseExact(qrCodeParts[3], "ddMMyyyy", CultureInfo.InvariantCulture);
                    }
                }
                catch (Exception ex)
                {
            ...
            {
                patient = await _unitOfWork.GetRepository<Patient>().GetAll()
                    .FirstOrDefaultAsync(item => item.IdentityCardNumber == nationalId);
                isSyncPatient = patient != null && name.Contains(patient.FirstName);
                if (isSyncPatient) return _mapper.Map<Patient, TempPatient>(patient);
            }
            ...
            for (var i = addressParts.Length - 1; i >= 0; i--)
            {
                ...
                if (indexFromEnd == 0)
                {
                    var mostMatchProvince = _unitOfWork.GetRepository<Province>().GetAll().ToList().Select(province =>
                            new Tuple<string, string, int>(
                                province.Code, province.Name,
                                CalculateDistance(StringUtils.NormalizeAddress(province.Name), normalizedAddress)))
                        .OrderBy(tup => tup.Item3).FirstOrDefault();
                    ...
                    var unidentifiedProvince = _unitOfWork.GetRepository<Province>().GetAll()
                        .Where(province => province.SearchText == unidentified).FirstOrDefault();
                ...
                if (indexFromEnd == 1)
                {
                    var mostMatchDistrict = _unitOfWork.GetRepository<District>().GetAll()
                        .Where(district1 => district1.ProvinceCode == location.ProvinceCode).ToList().Select(district =>
                            new Tuple<string, string, int>(
                                district.Code, district.Name,
                                CalculateDistance(StringUtils.NormalizeAddress(district.Name), normalizedAddress)))
                        .OrderBy(tup => tup.Item3).FirstOrDefault();
                    ...
                    var unidentifiedDistrict = _unitOfWork.GetRepository<District>().GetAll().Where(district =>
                            district.ProvinceCode == location.ProvinceCode && district.SearchText == unidentified)
                        .FirstOrDefault();
                ...
                if (indexFromEnd == 2)
                {
                    var mostMatchWard = _unitOfWork.GetRepository<Ward>().GetAll()
                        .Where(ward1 => ward1.DistrictCode == location.DistrictCode).ToList().Select(ward1 =>
                            new Tuple<string, string, int>(
                                ward1.Code, ward1.Name,
                                CalculateDistance(StringUtils.NormalizeAddress(ward1.Name), normalizedAddress)))
                        .OrderBy(tup => tup.Item3).FirstOrDefault();
                    ...
                    var unidentifiedWard = _unitOfWork.GetRepository<Ward>().GetAll().Where(ward =>
                        ward.DistrictCode == location.DistrictCode && ward.SearchText == unidentified).FirstOrDefault();
            ...
            address = !string.IsNullOrEmpty(location.ProvinceCode) ? string.Join("", addressParts.Where(s => s != "")) : parts[5];
        ...
        public async Task<TempPatient> GetPatientByQRCodeWithNoCheckSocialGateway(string code)
        {
            ...
            patient = await _unitOfWork.GetRepository<Patient>().GetAll()
                .FirstOrDefaultAsync(item => item.HealthInsuranceNumber == insuranceCode);
            if (patient != null) return _mapper.Map<Patient, TempPatient>(patient);
        ...
        public async Task<TableResultJsonResponse<PatientInfoViewModel>> GetAllAsync(DataTableParameters parameters)
        {
            ...
            if (!string.IsNullOrEmpty(parameters.Search.Value))
            {
                ...
                predicate = predicate.Or(p =>
                    !string.IsNullOrEmpty(p.LastName) && p.LastName.ToLower().Contains(searchValue));
                predicate = predicate.Or(p =>
                    !string.IsNullOrEmpty(p.FirstName) && p.FirstName.ToLower().Contains(searchValue));
            }
            ...
            foreach (var patient in filteredPatients)
            {
                ...
                var district = districts.FirstOrDefault(d =>
                    d.Code == patient.DistrictCode && d.ProvinceCode == patient.ProvinceCode);
                var ward = wards.FirstOrDefault(w =>
                    w.Code == patient.WardCode && w.DistrictCode == patient.DistrictCode);
        ...
        public async Task<PatientInfoViewModel> GetPatientByHealthInsuranceNumber(string healthInsuranceNumber)
        {
            var patient = await _unitOfWork.GetRepository<Patient>()
                .FindAsync(x => x.HealthInsuranceNumber == healthInsuranceNumber);
        ...
        public async Task<Patient> GetByCode(string code, InputTypeCard cardType = InputTypeCard.None)
        {
            ...
            var tempPatient = await _unitOfWork.GetRepository<TempPatient>().GetAll()
                .FirstOrDefaultAsync(item => item.Code == code);
            if (tempPatient != null) return _mapper.Map<TempPatient, Patient>(tempPatient);

            if (cardType == InputTypeCard.None && length == 0) return null;
            ...
            if (cardType == InputTypeCard.None && length > 0)
            {
                if (length == 8)
                {
                    switch (cardType)
                    {
                        case InputTypeCard.RegisteredCode:
                            cardType = InputTypeCard.RegisteredCode;
                            var reception = await _unitOfWork.GetRepository<Reception>().GetAll()
                                .FirstOrDefaultAsync(item => item.RegisteredCode == code);
                            if (reception != null) patient = reception.Patient;
                            break;
                        ...
                        default:
                            cardType = InputTypeCard.Unknown;
                            patient = await _unitOfWork.GetRepository<Patient>().GetAll()
                                .FirstOrDefaultAsync(item => item.Code == code);
                            if (patient == null)
                            {
                                var r = await _unitOfWork.GetRepository<Reception>().GetAll()
                                    .FirstOrDefaultAsync(item => item.RegisteredCode == code);
                                if (r != null) patient = r.Patient;
                            }
                ...
                else if (length == 12)
                {
                    ...
                    patient = await _unitOfWork.GetRepository<Patient>().GetAll()
                        .FirstOrDefaultAsync(item => item.IdentityCardNumber == code);
                }
                else
                {
                    cardType = InputTypeCard.HealthInsuranceCard;
                    patient = await _unitOfWork.GetRepository<Patient>().GetAll()
                        .FirstOrDefaultAsync(item => item.HealthInsuranceNumber == code);
                }
            }
            ...
            if (patient == null)
            {
                ...
                if (response != null)
                {
                    ...
                    var patientModel = _mapper.Map<Patient>(response);
                    _unitOfWork.GetRepository<Patient>().Add(patientModel);
                    await _unitOfWork.CommitAsync();
                    return patientModel;
                }
            }
        ...
        {
            var reception = await _unitOfWork.GetRepository<Reception>().GetAll()
                .FirstOrDefaultAsync(x => x.RegisteredCode == registerCode);
            if (reception == null) throw new Exception($"Không tìm thấy thông tin với số điều trị {registerCode}");
            ...
            if (patient == null)
            {
                ...
                if (string.IsNullOrEmpty(response.DateOfBirth) == false)
                {
                    ...
                    {
                        var birthDateStr = response.DateOfBirth.Substring(0, 8);
                        var birthDate = DateTime.MinValue;
                        if (DateTime.TryParseExact(birthDateStr, "yyyyMMdd", CultureInfo.InvariantCulture,
                                DateTimeStyles.None, out birthDate)) patient.Birthday = birthDate;
                    }
                }
                else
                {
                    ...
                    {
                        var year = Convert.ToInt32(response.YearOfBirth);
                        if (year > 0) patient.Birthday = new DateTime(year, 1, 1);
                    }
                }
        ...
        public async Task<GetPatientByQRCodeViewModel> GetPatientByQRCode(GetPatientByQRCodeRequest request)
        {
            ...
            if (InsuranceCode == "HC4790019302065") address = "Phường 4, Tân Bình, Hồ Chí Minh";
        ...
        public async Task<PatientInfoViewModel> GetPatientTypeByRegisteredCode(
            GetPatientByRegisteredCodeRequest request)
        {
                ...
                if (patient == null)
                {
                    if (string.IsNullOrEmpty(response.DateOfBirth) == false)
                    {
                        if (response.DateOfBirth.Length >= 8)
                        {
                            ...
                            var birthDate = DateTime.MinValue;
                            if (DateTime.TryParseExact(birthDateStr, "yyyyMMdd", CultureInfo.InvariantCulture,
                                    DateTimeStyles.None, out birthDate)) patientFromHis.Birthday = birthDate;
                        }
                    }
                    else
                    {
                        ...
                        {
                            var year = Convert.ToInt32(response.YearOfBirth);
                            if (year > 0) patientFromHis.Birthday = new DateTime(year, 1, 1);
                        }
                    }
                    ...
                    if (string.IsNullOrEmpty(response.HealthInsuranceIssuedDate) == false)
                        ...
                        {
                            var healthInsuranceIssuedDateStr = response.HealthInsuranceIssuedDate.Substring(0, 8);
                            var healthInsuranceIssuedDate = DateTime.MinValue;
                            if (DateTime.TryParseExact(healthInsuranceIssuedDateStr, "yyyyMMdd",
                                    CultureInfo.InvariantCulture, DateTimeStyles.None, out healthInsuranceIssuedDate))
                    ...
                    if (string.IsNullOrEmpty(response.HealthInsuranceExpiredDate) == false)
                        ...
                        {
                            var healthInsuranceExpiredDateStr = response.HealthInsuranceExpiredDate.Substring(0, 8);
                            var healthInsuranceExpiredDate = DateTime.MinValue;
                            if (DateTime.TryParseExact(healthInsuranceExpiredDateStr, "yyyyMMdd",
                                    CultureInfo.InvariantCulture, DateTimeStyles.None, out healthInsuranceExpiredDate))
        ...
        {
            var array = new int[first.Count() + 1, second.Count() + 1];

            for (var index = 0; index < first.Count() + 1; index++) array[index, 0] = index;

            for (var index = 0; index < second.Count() + 1; index++) array[0, index] = index;

            for (var index = 1; index < first.Count() + 1; index++)
            for (var rowIndex = 1; rowIndex < second.Count() + 1; rowIndex++)
        ...
        /// <returns></returns>
        private PatientInfoViewModel ToPatientInfoViewModel(Patient data, Province province, District district,
            Ward ward)
        {
        ...
        /// <param name="ward">The ward.</param>
        /// <returns></returns>
        private PatientInfoViewModel ToPatientInfoViewModel(ExportRequest request, Patient data, Province province,
        ...
        {
            if (!string.IsNullOrEmpty(property)) property = property.Trim().ToUpper();

            if (property == $"{SortCondition.PATIENT_CODE}{SortCondition.ORDER_BY_ESC}")
                return query.OrderBy(p => p.Code);
            if (property == $"{SortCondition.PATIENT_CODE}{SortCondition.ORDER_BY_DESC}")
                return query.OrderByDescending(p => p.Code);
            if (property == $"{SortCondition.NAME}{SortCondition.ORDER_BY_ESC}")
                return query.OrderBy(p => p.FullName);
            if (property == $"{SortCondition.NAME}{SortCondition.ORDER_BY_DESC}")
                return query.OrderByDescending(p => p.FullName);
            if (property == $"{SortCondition.TEKMEDICARD}{SortCondition.ORDER_BY_ESC}")
                return query.OrderBy(p => p.TekmediCardNumber);
            if (property == $"{SortCondition.TEKMEDICARD}{SortCondition.ORDER_BY_DESC}")
