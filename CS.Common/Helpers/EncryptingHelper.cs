...
namespace CS.Common.Helpers
{
    ...
    public static class EncryptingHelper
    {
        ...
        public static string AesEncrypt(string value, string key, string salt)
        {
            ...
            var rgbKey = rgb.GetBytes(256 >> 3);
            var rgbIV = rgb.GetBytes(128 >> 3);
        ...
        public static string AesEncrypt(string value, byte[] rgbKey, byte[] rgbIV, Encoding encoding)
        {
            ...
            {
                var transform = aes.CreateEncryptor(rgbKey, rgbIV);
        ...
        public static string AesDecrypt(string text, byte[] rgbKey, byte[] rgbIV, Encoding encoding)
        {
            using (var aes = Aes.Create())
            {
                var transform = aes.CreateDecryptor(rgbKey, rgbIV);
        ...
        public static bool Hmac256Verify(byte[] secretkey, Stream inStream, string hashedValue)
        {
            ...
            using (var hmac = new HMACSHA256(secretkey))
            {
                ...
                var computedValue = ByteArrayToHexString(computedHash);
        ...
        /// <returns><c>true</c> if XXXX, <c>false</c> otherwise.</returns>
        public static bool RSACheckSign(string signedMessage, string originalMessage, string publicKey,
            Encoding encoding)
        {
            var pr = new PemReader(new StringReader(publicKey));
            var rsaParams = DotNetUtilities.ToRSAParameters((RsaKeyParameters)pr.ReadObject());
        ...
        /// <param name="encoding">Encoding of original content</param>
        /// <returns><c>true</c> if XXXX, <c>false</c> otherwise.</returns>
        public static bool RSACheckSign(string signedMessage, string originalMessage, RSAParameters publicKey,
            Encoding encoding)
        {
            var success = false;
            using (var rsa = new RSACryptoServiceProvider())
            {
                ...
                var bytesToVerify = encoding.GetBytes(originalMessage);
        ...
        public static string RSASign(string content, string privateKey, Encoding encoding)
        {
            ...
            var signedBytes = RSASign(contentBytes, privateKey);
        ...
        {
            var contentBytes = encoding.GetBytes(content);
            var signedBytes = RSASign(contentBytes, rsaParams);
            return Convert.ToBase64String(signedBytes);
        }
        ...
        public static byte[] RSASign(byte[] content)
        {
            ...
            var pr = new PemReader(File.OpenText("TEK_dev_privatekey.pem"));
            var KeyPair = (AsymmetricCipherKeyPair)pr.ReadObject();
            var rsaParams = DotNetUtilities.ToRSAParameters((RsaPrivateCrtKeyParameters)KeyPair.Private);
