...
namespace TEK.Payment.Domain.Services
{
    ...
    public class ReExaminationService : IReExaminationService
    {
        ...
        public async Task<ICollection<ExaminationCalendar>> GetListByCodeAndDate(GetReExaminationListByCodeAndDateModel model, bool isAuto = false)
        {
            var reExams = await _hospitalSystemService.GetRawListReExaminationByCodeAndDate(new GetRawReExaminationListByCodeAndDateRequest
            {
            ...
            foreach (var item in reExams)
            {
                ...
                if (activeDept != null && !activeDept.IsActive)
                {
                    continue;
                }

                var calendar = await _unitOfWork.GetRepository<ExaminationCalendar>().GetAll().FirstOrDefaultAsync(c => c.PatientCode == item.PatientCode
        ...
        public async Task<ICollection<QueueNumber>> Create(Patient patient, List<ReExamRegistrationListModel> model, bool isThrowError = false)
        {
            ...
            {
                result = _unitOfWork.GetRepository<QueueNumber>().GetAll().Where(q => bookingIds.Any(id => id == q.BookingId)).ToList();
            }
            else
            {
                foreach (var item in model)
                {
                    ...
                    var number = await _unitOfWork.GetRepository<QueueNumber>().GetAll().Where(q => q.DepartmentCode == item.DepartmentCode
            ...
            var reExamsResp = await _hospitalSystemService.PostRawRegisterReExamination(new PostRawRegisterReExaminationRequest
            {
                ...
                Departments = model.Select(_mapper.Map<ReExamRegistrationListModel, PostRawRegisterReExaminationDataRequest>).ToList()
            });
        ...
        public async Task<ICollection<QueueNumber>> CreateAndGenerateNumber(Patient patient, List<ReExamRegistrationListModel> model)
        {
            var result = new List<QueueNumber>();
        ...
        public async Task<ICollection<ExaminationCalendar>> GetManualListByCodeAndDate(GetReExaminationListByCodeAndDateModel model, bool isAuto = false)
        {
            var reExams = await _hospitalSystemService.GetRawListReExaminationByCodeAndDate(new GetRawReExaminationListByCodeAndDateRequest
            {
                PatientCode = model.PatientCode,
            ...
            if (reExams.Count == 0)
            {
                ...
                if (result.Count == 0)
                {
                    throw new Exception(ErrorMessages.NotYetRegisteredExam);
                }
